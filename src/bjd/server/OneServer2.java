...
//****************************************************************
public abstract class OneServer2 extends ThreadBase implements ISock {
	...
	//コンストラクタ
	protected OneServer2(Kernel kernel, String nameTag, Conf conf, OneBind oneBind) {
	...
	protected final void onRunThread() {
		...
		logger.set(LogKind.Normal, (SockBase)null, 9000000, bindStr);
		...
		if (sockServer.getSockState() == SockState.Error) {
			logger.set(LogKind.Error, (SockBase)null, 9000035, sockServer.getLastEror()); //Socket生成でエラーが発生しました。[TCP]
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
				e.printStackTrace();
	...
	public final void accept(final SocketChannel accept, SockServer sockServer) {
		...
		if (count() >= multiple) {
			...
			logger.set(LogKind.Secure, (SockBase)null, 9000004, String.format("count:%d/multiple:%d", count(), multiple));
		} else {
			//子ソケット作成
