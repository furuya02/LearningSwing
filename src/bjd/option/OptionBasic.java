...
public class OptionBasic extends OneOption {
	...
	public OptionBasic(Kernel kernel) {
		super(kernel);

		//基本オプションの場合、この時点でkernelのJapanは、まだ読み込まれていないので、強制的に初期化する
		//        var iniDb = new IniDb(kernel.ProgDir(), "Option");
		//        var lv = new ListVal();
		//        lv.Add(new OneVal("lang", 0,Crlf.Nextline,new CtrlComboBox("Language",new List<string>{"Japanese", "English"})));
		//        iniDb.Read("Basic",lv);
	...
	private OnePage page1(String name, String title) {
		...
		onePage.add(new OneVal("password", "", Crlf.NEXTLINE, new CtrlHidden(kernel.getJp() ? "管理者パスワード" : "password", 200)));
		onePage.add(new OneVal("serverName", "", Crlf.NEXTLINE, new CtrlTextBox(kernel.getJp() ? "サーバ名" : "Server Name", 200)));
