...
public final class Option extends OneOption {
	...
	public Option(Kernel kernel) {
		...
		add(new OneVal("useServer", true, Crlf.NEXTLINE, new CtrlCheckBox(
				"サーバを使用する")));
	...
	private OnePage page1(String name, String title) {
		...
		onePage.add(new OneVal("useDat", true, Crlf.NEXTLINE, new CtrlCheckBox(
				"DatCtrlの有効無効")));

		ListVal list = new ListVal();
		list.add(new OneVal("combo", 0, Crlf.NEXTLINE, new CtrlComboBox(
				"コンボボックス", new String[] { "DOWN", "PU", "FULL" }, 200)));
		list.add(new OneVal("fileName2", "c:\\work", Crlf.NEXTLINE,
				new CtrlFolder("フォルダ", 30, kernel)));
		list.add(new OneVal("text", "user1", Crlf.NEXTLINE, new CtrlTextBox(
				"テキスト入力", 30)));
		list.add(new OneVal("hidden", "123", Crlf.NEXTLINE, new CtrlHidden(
				"パスワード", 30)));
		onePage.add(new OneVal("dat", null, Crlf.NEXTLINE, new CtrlDat("データ",
				list, 300, kernel)));
	...
	private OnePage page2(String name, String title) {
		...
		onePage.add(new OneVal("useTextBox1", true, Crlf.NEXTLINE,
				new CtrlCheckBox("チェック")));
		onePage.add(new OneVal("n2", true, Crlf.NEXTLINE, new CtrlCheckBox(
				"チェック2")));
		onePage.add(new OneVal("textBox1", "123", Crlf.NEXTLINE,
				new CtrlTextBox("文字列入力", 20)));
	...
	private OnePage page3(String name, String title) {
		...
		onePage.add(new OneVal("n4", "123", Crlf.NEXTLINE, new CtrlHidden(
				"パスワード", 20)));
